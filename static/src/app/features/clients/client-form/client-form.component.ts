...
@Component({
  ...
  template: `
    <div class="max-w-2xl mx-auto space-y-6">
      <!-- Loading Spinner -->
      <app-loading-spinner
        *ngIf="loadingState.isLoading && !clientForm"
        [showText]="true"
      ...
      <!-- Form -->
      <div *ngIf="clientForm" class="bg-white dark:bg-gray-900 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        <form [formGroup]="clientForm" (ngSubmit)="onSubmit()" class="p-6 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div *ngIf="clientForm.get('clientName')?.invalid && clientForm.get('clientName')?.touched"
                ...
                <div *ngIf="clientForm.get('clientName')?.errors?.['required']">Client name is required</div>
                <div *ngIf="clientForm.get('clientName')?.errors?.['minlength']">Client name must be at least 2 characters</div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              ></textarea>
              <div *ngIf="clientForm.get('clientPublicKey')?.invalid && clientForm.get('clientPublicKey')?.touched"
                   class="mt-1 text-sm text-red-600">
                <div *ngIf="clientForm.get('clientPublicKey')?.errors?.['pattern']">Invalid public key format</div>
              </div>
            </div>
            ...
            <div>
              ...
              <div *ngIf="clientForm.get('presharedKey')?.invalid && clientForm.get('presharedKey')?.touched"
                   class="mt-1 text-sm text-red-600">
                <div *ngIf="clientForm.get('presharedKey')?.errors?.['pattern']">Invalid pre-shared key format</div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div formArrayName="addresses" class="space-y-3">
              <div *ngFor="let address of addresses.controls; let i = index" class="space-y-2">
                <div [formGroupName]="i" class="flex items-start gap-3">
                  <div class="flex-1">
                    ...
                    />
                    <div *ngIf="address.get('address')?.invalid && address.get('address')?.touched"
                         class="mt-1 text-sm text-red-600">
                      <div *ngIf="address.get('address')?.errors?.['required']">IP address is required</div>
                      <div *ngIf="address.get('address')?.errors?.['pattern']">Invalid IP address format (use CIDR notation)</div>
                    </div>
                  </div>
                  <button
                    type="button"
          ...
          <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            >
              ...
              <span *ngIf="!submitting">Add Client</span>
            </button>
          </div>
        </form>
      </div>
