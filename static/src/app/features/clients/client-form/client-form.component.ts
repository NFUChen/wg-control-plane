              ...
              <div class="space-y-3">
                ...
                @if (isEditMode) {
                  <p class="text-xs text-blue-600 dark:text-blue-400">
                    ...
                    Peer IP addresses can now be modified after client creation.
                  </p>
                }
              </div>
            </div>
...
export class ClientFormComponent implements OnInit, OnDestroy {
  ...
  private loadClientForEdit(): void {
    ...
    this.wireguardService.getClientDetails(this.clientId).subscribe({
      next: (details) => {
        ...
        // Clear existing peer IPs and populate with current values
        while (this.peerIPs.length) {
          this.peerIPs.removeAt(0);
        }
        if (details.peerIPs?.length) {
          details.peerIPs.forEach(ip => this.addPeerIP(ip));
        } else {
          this.addPeerIP(''); // Ensure at least one empty field
        }

        // Clear and populate allowed IPs
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.clientId) {
      // Extract peer IPs from form
      const peerIPs = (formValue.peerIPs as { address: string }[])
        .map(a => a.address?.trim())
        .filter((a): a is string => !!a)
        .map(address => ({ address }));

      // Extract allowed IPs from form
      const allowedIPs = (formValue.allowedIPs as { address: string }[])
        .map(a => a.address?.trim())
        .filter((a): a is string => !!a)
        .map(address => ({ address }));
