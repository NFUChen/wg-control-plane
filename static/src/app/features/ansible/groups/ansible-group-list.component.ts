...
import { AnsibleInventoryGroup } from '../../../models/ansible.interface';
import { LoadingState, TableColumn } from '../../../models/wireguard.interface';

@Component({
  selector: 'app-ansible-group-list',
  standalone: true,
  imports: [CommonModule, RouterModule, DataTableComponent, AlertComponent, StatusBadgeComponent],
  template: `
    <div class="space-y-6">
      <a routerLink="/ansible" class="text-sm font-medium text-blue-600 dark:text-blue-400">← Ansible</a>
      ...
      <app-data-table
        ...
        subtitle="Ansible inventory groups and variables"
        [columns]="columns"
        [data]="filtered"
...
export class AnsibleGroupListComponent implements OnInit, OnDestroy {
  filtered: AnsibleInventoryGroup[] = [];
  private all: AnsibleInventoryGroup[] = [];
  ...
  primaryAction = { label: 'New group', icon: 'M12 4v16m8-8H4' };
  ...
  goNew(): void {
    this.router.navigate(['/ansible/groups/new']);
  }

  onRowAction(ev: { action: string; item: AnsibleInventoryGroup }): void {
    if (ev.action === 'edit') {
      this.router.navigate(['/ansible/groups', ev.item.id, 'edit']);
    }
    if (ev.action === 'delete') {
      if (confirm(`Delete group "${ev.item.name}"?`)) {
        this.ansible.deleteGroup(ev.item.id).subscribe({
          next: () => {
